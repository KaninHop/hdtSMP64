...
# oneTBB (TBB >= 2021) removed task_scheduler_init.  Port btThreads.cpp to the
# oneTBB API: use tbb::global_control for thread-count limiting and
# tbb::info::default_concurrency() instead of the removed default_num_threads().
...
string(
	REPLACE
		...
		"tbb::global_control* m_tbbSchedulerInit;"
		THREADS_CPP
		"${THREADS_CPP}")
string(
	REPLACE
...
if("multithreading" IN_LIST FEATURES)
	# Use TBB (from vcpkg) instead of PPL so Bullet's internal task scheduler
	# shares the same threading backend as the hdtSMP64 plugin.
	# vcpkg's oneTBB ships as tbb12(_debug).lib; bullet's CMake does a fixed
	# find_library(TBB_LIBRARY tbb ...), so we pre-seed the cache vars to
	# short-circuit the find and point at the right file per config.
	list(APPEND FEATURE_OPTIONS
		-DBULLET2_USE_TBB_MULTITHREADING=ON
		"-DBULLET2_TBB_INCLUDE_DIR=${CURRENT_INSTALLED_DIR}/include")
	list(APPEND _bullet_tbb_options_release
		"-DBULLET2_TBB_LIB_DIR=${CURRENT_INSTALLED_DIR}/lib"
		"-DTBB_LIBRARY=${CURRENT_INSTALLED_DIR}/lib/tbb12.lib"
		"-DTBBMALLOC_LIBRARY=${CURRENT_INSTALLED_DIR}/lib/tbbmalloc.lib")
	list(APPEND _bullet_tbb_options_debug
		"-DBULLET2_TBB_LIB_DIR=${CURRENT_INSTALLED_DIR}/debug/lib"
